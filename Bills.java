...
public class Bills {
   ...
   static class Bill {
      ...
      public Bill(String un, long p , String dt , String t , double ttl , boolean lpu , boolean cpu, ArrayList<Integer> dish , ArrayList<Integer> qnty , ArrayList<Double> values  ) {
         uname = un;
         ph = p;
         date = dt;
         time = t;
      ...
      static void fullBill() {
         ...
         System.out.println("\n" + "-".repeat(Menu.mxD + 40));
         System.out.println(String.format("%-10s%-" + Menu.mxD + "s%-"+Menu.mxP+"s%-10s%-20s","SL" , "DISH" , "PRICE" , "QTY" , "VALUE"));
         System.out.println("-".repeat(Menu.mxD + 40));

         System.out.println("ds :" + dishes.size()); // ! DEBUGGING

         for(int i = 0; i < dishes.size(); i++)
            System.out.println(String.format("%-10s%-" + Menu.mxD + "s%-" + Menu.mxP +"s%-10s%-20s" , (i+1) , Menu.dish.get(dishes.get(i)) ,Menu.price.get(dishes.get(i)) , qty.get(i) , value.get(i)));
   ...
   static void viewBills() {
      ...
      for(int i = 0; i < bills.size(); i++)
         System.out.println(String.format("%-5s%-" + mxN + "s%-20s%-10s%-10s", (i + 1) , bills.get(i).uname , bills.get(i).ph , bills.get(i).total , lp.get(ph.indexOf(bills.get(i).ph)) ));
   }
